...
export default {
  ...
  watch: {
    'config.rating': {
      immediate: true, // Isso fará com que a função seja chamada imediatamente após a criação do componente
      handler(newRating) {
        // Verifique se newRating é um número válido antes de chamar setStars
        if (!isNaN(newRating)) {
          this.setStars();
        }
      },
